...
# 1. Load the 0.6.4 Strategy Lab ranking override after the legacy strategy module.
...
# 2. Keep updater health validation inside the existing backup/restore transaction.
...
# 3. Dashboard reads the same ranking model and never presents inconclusive as score=0 evidence.
...
$dashboardText=Read-NrText $dashboard
if (-not $dashboardText.Contains('points = points.Where(delegate(StrategyPoint point) { return !string.Equals(point.RankingState, "inconclusive"')) {
    Replace-NrRequired $dashboard @'
...
# 4. Run the real historic migration evidence gate in Windows PR and release workflows.
$validate='.github/workflows/validate.yml'
if (-not (Read-NrText $validate).Contains('Test-Updater063MigrationEvidence.ps1')) {
    Replace-NrRequired $validate "      - name: Upload logs and smoke package`n" @'
      - name: Prove real 0.6.2 to 0.6.3 updater migration and rollback
        shell: pwsh
        run: |
          $migration = ./scripts/Test-Updater063MigrationEvidence.ps1 -OutputPath ./updater-063-migration-evidence.json
          if ($migration.status -ne 'passed') { throw 'Real updater migration evidence gate did not pass.' }
          if (-not $migration.forcedHealthFailureRollback) { throw 'Forced health-check rollback was not proven.' }
          if (-not $migration.pathHasSpaces -or -not $migration.pathHasNonAscii -or $migration.absolutePathPublished) { throw 'Migration path/privacy evidence is incomplete.' }

      - name: Upload logs and smoke package
'@
    Replace-NrRequired $validate "            package-offline.log`n            artifacts/NexRoute-0.6.3-win-x64.zip" "            package-offline.log`n            updater-063-migration-evidence.json`n            artifacts/NexRoute-0.6.3-win-x64.zip"
}
$release='.github/workflows/release.yml'
if (-not (Read-NrText $release).Contains('Test-Updater063MigrationEvidence.ps1')) {
    Replace-NrRequired $release "      - name: Build finalized package from immutable pinned upstream`n" @'
      - name: Prove real 0.6.2 to 0.6.3 updater migration and rollback
        shell: pwsh
        run: |
          $migration = ./scripts/Test-Updater063MigrationEvidence.ps1 -OutputPath ./updater-063-migration-evidence.json
          if ($migration.status -ne 'passed' -or -not $migration.forcedHealthFailureRollback) { throw 'Real updater migration evidence gate did not pass.' }

      - name: Build finalized package from immutable pinned upstream
'@
}

# 5. Repository contract requires all new hardening surfaces.
...
# 6. Remove bootstrap artifacts and restore least-privilege validation workflow before the commit.
$validateText=Read-NrText $validate
$pattern='(?ms)^\s{6}# BEGIN NEXROUTE 064 BOOTSTRAP\r?\n.*?^\s{6}# END NEXROUTE 064 BOOTSTRAP\r?\n'
$validateText=[regex]::Replace($validateText,$pattern,'')
$validateText=$validateText.Replace("permissions:`n  contents: write # temporary 0.6.4 materializer`n","permissions:`n  contents: read`n")
Write-NrText $validate $validateText

$obsoleteWorkflow=Join-Path $root '.github/workflows/nexroute-064-materialize.yml'
if (Test-Path -LiteralPath $obsoleteWorkflow -PathType Leaf) { Remove-Item -LiteralPath $obsoleteWorkflow -Force }

# Self-delete: this helper exists only to make the integration commit from the trusted validation workflow.
