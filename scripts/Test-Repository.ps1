...
$required=@(
    ...
    'overlay/.service/nexroute-services.ps1','overlay/.service/services.json',
    ...
    'docs/RELEASE_0.6.0_ACCEPTANCE.md','docs/UPDATES.md','docs/ATTESTATIONS.md','docs/RELEASES.md',
...
Assert-True ($readme -match [regex]::Escape('NexRoute 0.6.2')) 'README documents NexRoute 0.6.2'
Assert-True ($readme -match [regex]::Escape('Hot Bug Fix')) 'README documents the hotfix purpose'
...
foreach ($asset in @('NexRoute-0.6.2-win-x64.zip','NexRoute-0.6.2-win-x64.zip.sha256','NexRoute-0.6.2-validation.json','NexRoute-0.6.2-validation.md')) {
    Assert-True ($readme -match [regex]::Escape($asset)) "README documents $asset"
...
$loader=Read-Text 'overlay/.service/next/nexroute-runtime-extensions.ps1'
...
Assert-True ($loader.IndexOf("'nexroute-hotfix-062.ps1'",[StringComparison]::Ordinal) -gt $loader.IndexOf("'nexroute-diagnostics-fixes.ps1'",[StringComparison]::Ordinal)) '0.6.2 hotfix loads after diagnostic fixes'
foreach ($token in @("['administrator']","['runtime']","['network']",'detected','[Console]::Title = "NexRoute $version"')) { Assert-True ($hotfix -match [regex]::Escape($token)) "Hotfix contains $token" }
Assert-True ($diagnosticFixes -match 'nexroute-updater-entry\.ps1') 'Interactive updates route through updater entry'
foreach ($token in @('NEXROUTE_LATEST_RELEASE_FIXTURE','Invoke-WebRequest','HttpWebRequest','curl.exe','New-NrFallbackReleaseMetadata','without the GitHub API')) { Assert-True ($updaterEntry -match [regex]::Escape($token)) "Updater entry contains $token" }
Assert-True ($updaterEntry -notmatch 'api\.github\.com') 'Updater entry never uses api.github.com'
Assert-True ($updaterEntry -notmatch 'effectiveMetadata\s*=\s*\$null') 'Updater entry cannot silently fall back to API metadata'
foreach ($token in @('NexRoute 0.6.2 Hot Fix Тест','diagnosticCompatibility','NEXROUTE_LATEST_RELEASE_FIXTURE','updaterFallbackVersion','service.bat','nexroute.bat','nexroute-update.cmd','PropertyNotFound')) { Assert-True ($launcherSmoke -match [regex]::Escape($token)) "Package smoke contains $token" }

$build=(Read-Text 'scripts/Build-Release.ps1')+(Read-Text 'scripts/Build-Package.ps1')
foreach ($token in @('upstream-lock.json','patch-report.json','Expected 23 tracked patch targets','UpstreamCachePath','UpstreamArchive','UpdaterEntryIncluded')) { Assert-True ($build -match [regex]::Escape($token)) "Build contract contains $token" }
...
foreach ($token in @('Validate NexRoute 0.6.2 sources','Build and test NexRoute 0.6.2','NexRoute-0.6.2-smoke','Test-WindowsLaunchers.ps1','diagnosticCompatibility','updaterFallbackVersion','UpstreamArchive','npm run typecheck','npm run build','actions/checkout@v6','actions/setup-node@v6','actions/upload-artifact@v7')) { Assert-True ($validate -match [regex]::Escape($token)) "Validation workflow contains $token" }
Assert-True ($validate -notmatch 'NexRoute-0\.6\.[01]-smoke|Expected 0\.6\.[01]|Build and test NexRoute 0\.6\.[01]') 'Validation workflow has no stale package contract'
foreach ($token in @('id-token: write','attestations: write','artifact-metadata: write','actions/attest@v4','gh attestation verify','actions/checkout@v6','actions/upload-artifact@v7','gh release create')) { Assert-True ($release -match [regex]::Escape($token)) "Release workflow contains $token" }
...
foreach ($token in @('Onmaynec/NexRoute','Service Matrix','Strategy Lab','gh attestation verify','prefers-reduced-motion','NEXT_PUBLIC_SITE_URL','getLatestStableRelease','SoftwareApplication')) { Assert-True ($websiteText -match [regex]::Escape($token)) "Website contains $token" }
