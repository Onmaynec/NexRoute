function Show-NexRouteMenu {
    ...
    if ((Get-NexRouteEnvironmentValue -Name 'NEXROUTE_UI_ANIMATE' -Fallback '0') -eq '1') {
        ...
        Write-NexRouteRule -Fill '=' -Color Cyan
        Write-Host ''
        Invoke-NexRouteAnimation -Label $script:Text.boot1 -Duration 140
        Invoke-NexRouteAnimation -Label $script:Text.boot2 -Duration 140
        Invoke-NexRouteAnimation -Label $script:Text.boot3 -Duration 140
        Invoke-NexRouteAnimation -Label $script:Text.boot4 -Duration 160
        Invoke-NexRouteAnimation -Label $script:Text.boot5 -Duration 120 -Color Green
...
function Get-NexRouteActionLabel {
    ...
    $map = @{
        ...
        ipset = 'menu5'; updatecheck = 'menu6'; payload = 'menu7';
        syncipset = 'menu8'; synchosts = 'menu9'; releases = 'menu10';
        diagnostics = 'menu11'; tests = 'menu12'; services = 'menu14'
    }
...
function Show-NexRouteAction {
    ...
    Invoke-NexRouteAnimation -Label 'Validating request' -Duration 120
    Invoke-NexRouteAnimation -Label 'Locking configuration state' -Duration 130
    Invoke-NexRouteAnimation -Label 'Preparing system module' -Duration 150
    Invoke-NexRouteAnimation -Label $script:Text.actionComplete -Duration 110 -Color Green
}
...
function Show-NexRouteLaunch {
    ...
    Invoke-NexRouteAnimation -Label $script:Text.launchRead -Duration 120
    ...
    $engineColor = if ($engineOk) { 'Green' } else { 'Red' }
    $driverColor = if ($driverOk) { 'Green' } else { 'Red' }

    Invoke-NexRouteAnimation -Label $script:Text.launchEngine -Duration 120 -Color $engineColor
    Invoke-NexRouteAnimation -Label $script:Text.launchDriver -Duration 120 -Color $driverColor
    Invoke-NexRouteAnimation -Label $script:Text.launchLists -Duration 130
    Invoke-NexRouteAnimation -Label $script:Text.launchCommand -Duration 130
    Invoke-NexRouteAnimation -Label $script:Text.launchTransfer -Duration 120 -Color Green
...
function Show-NexRoutePayloadManager {
    ...
    while ($true) {
        ...
        for ($index = 0; $index -lt $files.Count; $index++) {
            Write-NexRouteOption -Number ($index + 1) -Label $files[$index].BaseName
        }
        Write-NexRouteRule -Fill '=' -Color Cyan
        if ($NonInteractive) { return }
        ...
        try {
            Invoke-NexRouteAnimation -Label 'Unlocking payload vault' -Duration 120
            Copy-Item -LiteralPath $files[$fileNumber - 1].FullName -Destination $destination -Force
            Invoke-NexRouteAnimation -Label 'Committing active payload' -Duration 120 -Color Green
...
function Invoke-NexRouteIpsetSwitch {
    ...
    Invoke-NexRouteAnimation -Label 'Snapshot current routing state' -Duration 130
    Invoke-NexRouteAnimation -Label $script:Text.transitionApply -Duration 180
    try {
        if ($target -eq 'none') {
...
function Invoke-NexRouteSyncIpSet {
    ...
    try {
        Invoke-NexRouteAnimation -Label $script:Text.syncResolve -Duration 120
        Invoke-NexRouteAnimation -Label $script:Text.syncDownload -Duration 150
        Invoke-WebRequest -Uri $url -OutFile $tempPath -UseBasicParsing -TimeoutSec 20
        Invoke-NexRouteAnimation -Label $script:Text.syncValidate -Duration 130
        ...
        Invoke-NexRouteAnimation -Label $script:Text.syncBackup -Duration 110
        if (Test-Path -LiteralPath $listPath -PathType Leaf) { Copy-Item -LiteralPath $listPath -Destination $backupPath -Force }
        Invoke-NexRouteAnimation -Label $script:Text.syncCommit -Duration 150
...
function Invoke-NexRouteSyncHosts {
    ...
    try {
        Invoke-NexRouteAnimation -Label $script:Text.syncResolve -Duration 120
        Invoke-NexRouteAnimation -Label $script:Text.syncDownload -Duration 150
        Invoke-WebRequest -Uri ($url + '?t=' + [DateTime]::UtcNow.Ticks) -OutFile $tempPath -UseBasicParsing -TimeoutSec 20
        Invoke-NexRouteAnimation -Label $script:Text.syncValidate -Duration 130
        ...
        if ($isCurrent) {
            Invoke-NexRouteAnimation -Label $script:Text.syncCommit -Duration 110 -Color Green
            Write-NexRouteResult -Success $true -Message $script:Text.syncNoChange
            Remove-Item -LiteralPath $tempPath -Force -ErrorAction SilentlyContinue
        }
        else {
            Invoke-NexRouteAnimation -Label $script:Text.syncBackup -Duration 110
            Write-NexRouteResult -Success $true -Message 'Remote hosts dataset downloaded. Complete the merge in Notepad.'
...
function Show-NexRouteTestsIntro {
    ...
    Invoke-NexRouteAnimation -Label $script:Text.testsStart -Duration 160
    Invoke-NexRouteAnimation -Label 'Checking PowerShell runtime' -Duration 120
    Invoke-NexRouteAnimation -Label 'Loading DPI test suite' -Duration 150
    Invoke-NexRouteAnimation -Label $script:Text.testsWindow -Duration 110 -Color Green
    Wait-NexRouteKey
}
