...
- включённые сервисы реально добавляются во **все 22 стратегии** — `general.bat` и каждый вариант `general (...)`;
...
- новый значок NexRoute генерируется из приложенного логотипа в многоразмерный `.ico` и назначается ярлыку;
...
Чем больше сервисов включено, тем дольше выполняется полный прогон 22 стратегий.
