Describe 'NexRoute secure updater' {
    ...
    It 'rolls back to the previous verified backup' {
        ...
        $json = & $script:updaterPath -Mode Rollback -Root $script:installedRoot -Json
        $result = ($json | Select-Object -Last 1) | ConvertFrom-Json
    ...
    It 'wires the launcher to the automatic updater' {
        $launcher = Get-Content -LiteralPath (Join-Path $script:repositoryRoot 'overlay/nexroute.bat') -Raw
        $pages = Get-Content -LiteralPath (Join-Path $script:repositoryRoot 'overlay/.service/i18n/nexroute-pages.ps1') -Raw
