Describe 'NexRoute 0.6.1 release coherence' {
    BeforeAll {
        ...
        $script:releaseNotes=Get-Content -LiteralPath (Join-Path $root '.github/release-notes/v0.6.1.md') -Raw -Encoding UTF8
    ...
    It 'uses 0.6.1 as the canonical package and website version' {
        $script:version | Should -Be '0.6.1'
        [string]$script:websitePackage.version | Should -Be $script:version
        $script:readme | Should -Match ([regex]::Escape('NexRoute 0.6.1'))
        $script:changelog | Should -Match ([regex]::Escape('## [0.6.1] - 2026-08-04'))
        $script:releaseNotes | Should -Match ([regex]::Escape('# NexRoute 0.6.1 — Hot Bug Fix'))
    }

    It 'uses 0.6.1 package names and job labels in the validation workflow' {
        foreach ($token in @(
            'Validate NexRoute 0.6.1 sources',
            'Build and test NexRoute 0.6.1',
            "if (`$version -ne '0.6.1')",
            'NexRoute-0.6.1-smoke',
            'artifacts/NexRoute-0.6.1-win-x64.zip',
            'artifacts/NexRoute-0.6.1-win-x64.zip.sha256'
        )) {
            $script:validateWorkflow | Should -Match ([regex]::Escape($token))
        }
        $script:validateWorkflow | Should -Not -Match 'NexRoute-0\.6\.0-smoke|Expected 0\.6\.0|Validate NexRoute 0\.6\.0|Build and test NexRoute 0\.6\.0'
    }

    It 'executes the launcher hotfix contract for online and offline packages' {
    ...
    It 'documents and verifies the four-subject 0.6.1 release trust flow' {
        foreach ($token in @(
