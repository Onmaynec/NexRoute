Describe 'NexRoute 0.6.5 release coherence' {
    BeforeAll {
        $root=Split-Path -Parent $PSScriptRoot
        $script:version=(Get-Content -LiteralPath (Join-Path $root '.service/version.txt') -Raw -Encoding UTF8).Trim()
        $script:releaseNotes=Get-Content -LiteralPath (Join-Path $root '.github/release-notes/v0.6.5.md') -Raw -Encoding UTF8
        $script:acceptance=Get-Content -LiteralPath (Join-Path $root 'docs/RELEASE_0.6.5_ACCEPTANCE.md') -Raw -Encoding UTF8
    ...
    It 'uses 0.6.5 as the canonical package and website version' {
        $script:version | Should -Be '0.6.5'
        ...
        $script:releaseNotes | Should -Match ([regex]::Escape('# NexRoute 0.6.5 — возвращение классического интерфейса и новый Strategy Lab'))
        $script:acceptance | Should -Match ([regex]::Escape('NexRoute 0.6.5 — критерии готовности релиза'))
    }

    It 'uses 0.6.5 package names and job labels in the validation workflow' {
        foreach ($token in @(
            'Validate NexRoute 0.6.5 sources',
            'Build and test NexRoute 0.6.5',
            "if (`$version -ne '0.6.5')",
            'NexRoute-0.6.5-smoke',
            'artifacts/NexRoute-0.6.5-win-x64.zip',
            'artifacts/NexRoute-0.6.5-win-x64.zip.sha256'
    ...
    It 'documents the four-subject 0.6.5 release trust flow' {
        foreach ($token in @('NexRoute-0.6.5-win-x64.zip','NexRoute-0.6.5-win-x64.zip.sha256','NexRoute-0.6.5-validation.json','NexRoute-0.6.5-validation.md')) { $script:releaseNotes | Should -Match ([regex]::Escape($token)) }
