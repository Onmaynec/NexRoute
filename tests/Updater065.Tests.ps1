Describe 'NexRoute 0.6.5 updater migration from 0.6.4' {
    BeforeAll {
        ...
        function New-Nr065Release {
            param([string]$Path,[string]$Version='0.6.5')
    ...
    It 'rolls back byte-preserved user state when the post-update health check fails' {
        ...
        try {
            New-Nr065InstalledRoot -Path $installed -Version '0.6.4'
            $fixture = New-Nr065Release -Path (Join-Path $testRoot 'release')
            $before = @{}
            ...
            (Get-Content -LiteralPath (Join-Path $installed '.service/version.txt') -Raw).Trim() | Should -Be '0.6.4'
    ...
    It 'updates successfully and a second install is an idempotent current result' {
        ...
        try {
            ...
            $first.CurrentVersion | Should -Be '0.6.5'
            ...
            $second.CurrentVersion | Should -Be '0.6.5'
