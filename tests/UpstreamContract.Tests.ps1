...
Describe 'NexRoute upstream contract' {
    BeforeEach {
        $testRoot = Join-Path ([System.IO.Path]::GetTempPath()) ('nexroute-upstream-test-{0}' -f [guid]::NewGuid().ToString('N'))
    ...
    function New-TestArchive {
        ...
        $archive = Join-Path $testRoot 'example-1.2.3.zip'
    ...
    It 'requires a locked digest unless bootstrap mode is explicit' {
        ...
        { Resolve-NexRouteUpstreamArchive -Manifest $manifest -WorkingDirectory (Join-Path $testRoot 'resolve') -ArchivePath $archive } |
    ...
    It 'resolves and verifies an offline archive using the locked digest' {
        ...
        $resolved = Resolve-NexRouteUpstreamArchive `
            -Manifest $manifest `
            -WorkingDirectory (Join-Path $testRoot 'resolve') `
            -ArchivePath $archive
