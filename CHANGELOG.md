...
- основной интерфейс возвращён к числовой и секционной логике NexRoute 0.2.2 без отката функциональности 0.6.4;
- добавлен новый одноцветный ASCII-логотип NEXROUTE и чёрно-красная палитра;
- пункт [12] Strategy Lab открывает новый classic-сценарий напрямую;
- расширенные функции 0.6.4 сохранены через Advanced Toolkit.

### Strategy Lab

- добавлены режимы проверки всех или выбранных стратегий;
- добавлен preflight для прав администратора, winws, WinDivert, сети и опционального curl;
- на каждый конфиг выполняются повторные DNS, TLS, HTTPS и ICMP проверки;
- добавлены streamed throughput, YouTube/media и DPI range checks;
- во время теста показываются OK/WARN/FAIL/SKIP, прогресс, раунд и детали измерений;
- JSON/CSV history и explainable ranking сохраняются, недостаточные данные остаются INCONCLUSIVE;
- после лаборатории восстанавливается исходная установленная стратегия.

### Release

- baseline остаётся Flowseal 1.10.3: 22 upstream-стратегии и 24 patch-targets;
- stable auto-update рассчитан на переход 0.6.4 → 0.6.5.
